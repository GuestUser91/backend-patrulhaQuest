...
export class DynamicService {
  static async createDynamic(data: CreateDynamicRequest) {
  ...
  static async getDynamicById(id: string) {
    const dyn = await prisma.dynamic.findUnique({
      where: { id },
      include: { questions: true, words: true, memoryCards: true },
    });

    if (!dyn) throw new Error('Dinâmica não encontrada');
    return dyn;
  }

  static async deleteDynamic(id: string) {
