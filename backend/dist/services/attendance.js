...
export class AttendanceService {
    static async recordAttendance(date, records) {
    ...
    static async getStats() {
        ...
        for (const g of groups) {
            ...
            if (!countsMap[sid])
                countsMap[sid] = { presentCount: 0, absentCount: 0 };
            if (g.present)
                countsMap[sid].presentCount = g._count._all;
            else
                countsMap[sid].absentCount = g._count._all;
        }
